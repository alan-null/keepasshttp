...
| `Hmac`          | string | Optional                         | Base64 HMAC-SHA256 over (IV + ciphertext). Optional on requests; responses include HMAC for verification. |
...
| `Hmac`        | string | Always          | Base64 HMAC-SHA256 over (IV + ciphertext) for authenticated encryption. |

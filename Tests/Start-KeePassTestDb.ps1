# Ensure KeePass test instance is running with --kph-test-config.
...
function Start-KeePassTest {
    ...
    & $keePassExe $dbPath -pw:test --kph-test-config
}
...
foreach ($proc in $existing) {
    if ($proc.CommandLine -match '--kph-test-config') {
        $hasTestConfig = $true
        break
    }
}
...
if ($hasTestConfig) {
    Write-Host "KeePass test instance already running with --kph-test-config."
    return
}

# No process has the test config; ask user before killing
Write-Host "KeePass is running without --kph-test-config."

...
namespace KeePassHttp
{
    public sealed partial class KeePassHttpExt : Plugin
    {
        ...
        private BaseResponse SetLoginHandler(BaseRequest br, Aes aes)
        {
            ...
            PwUuid uuid = null;
            string name = r.Name.DecryptString(aes);
            string username = r.Login.DecryptString(aes);
            string password = r.Password.DecryptString(aes);
            string url = r.Url.DecryptString(aes);
            string realm = r.Realm.DecryptString(aes);
            var config = realm != null ? new KeePassHttpEntryConfig { Realm = realm, Allow = null, Deny = null } : null;
            var stringFields = r.StringFields.DecryptDictionary(aes);
            ...
            if (uuid != null)
            {
                ...
                var model = new UpdateEntryModel
                {
                    Uuid = uuid,
                    Title = name,
                    Username = username,
                ...
                resp.Success = UpdateEntry(model, r.Id);
                resp.Uuid = r.Uuid;
            }
            else
            {
                // create new entry
                string urlHost = url != null ? GetHost(url) : null;
                string submitUrl = r.SubmitUrl.DecryptString(aes);
                var submithost = submitUrl != null ? GetHost(submitUrl) : null;

                var model = new CreateEntryModel
                {
                    Title = name == null ? urlHost : name,
                    Username = username,
                    Password = password,
                    Url = url.RemoveUrlParameters(),
                    Config = config,
                    StringFields = stringFields
                };
                var pw = CreateEntry(model, submithost);
                resp.Success = pw != null;
                resp.Uuid = pw != null ? MemUtil.ByteArrayToHexString(pw.UuidBytes).EncryptString(aes) : null;
            }
        ...
        private BaseResponse GeneratePassword(BaseRequest br, Aes aes)
        {
            ...
            PwGenerator.Generate(out psNew, autoProfile, null, Program.PwGeneratorPool);
        ...
        private bool UpdateEntry(UpdateEntryModel model, string keyId)
        {
            PwEntry entry = null;
            ...
            foreach (PwDatabase db in GetDatabases(configOpt))
            {
                entry = db.RootGroup.FindEntry(model.Uuid, true);
                if (entry != null)
                {
                    break;
                }
            ...
            if (!allowUpdate)
            {
                ...
                result = MessageBox.Show(
                    host.MainWindow.IsTrayed() ? null : host.MainWindow,
                    message,
                    title,
                    MessageBoxButtons.YesNo,
            ...
            if (allowUpdate)
            {
                ...
                var updated = entry.SetEntry(model);
                if (updated)
                {
        ...
        private PwUuid CreateEntry(CreateEntryModel model, string submithost)
        {
            if (model == null)
            {
                return null;
            ...
            string realm = model.Config != null ? model.Config.Realm : null;
